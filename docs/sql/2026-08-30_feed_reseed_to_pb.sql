...
BEGIN
    ...
    IF n > 0 THEN
        RAISE EXCEPTION
          'STOP: % feed_usage row(s) exist. Cost has frozen against these layers. '
          'Re-seeding would rewrite history - use a physical count instead.', n;
    END IF;
    ...
    IF n > 0 THEN
        RAISE EXCEPTION
          'STOP: % posted count(s) exist. The books have been counted once already - '
          'correct with another count, not by re-seeding.', n;
    END IF;
...
-- A silently unmatched row would leave that commodity on its old figure
-- while everything around it moved - the worst possible outcome.
-- ---------------------------------------------------------------------
DO $match$
DECLARE missing text; dupes text;
BEGIN
...
DECLARE bad text;
BEGIN
    SELECT string_agg(item_name, ', ') INTO bad
      FROM target WHERE new_qty_lb > 0 AND location_id IS NULL;
...
-- Safe to DELETE outright only because guard 0 proved nothing references
-- these layers: no feed_usage means no feed_usage_costs.
-- ---------------------------------------------------------------------
...
SELECT
    ...
    t.location_id,
    '(opening balance)',
    'Opening balance re-seeded from the Performance Beef inventory screen dated '
      || '2026-08-30 at $' || t.cost_per_ton::text || '/ton. '
      || CASE WHEN t.pb_raw_qty_lb < 0
              THEN 'PB carried ' || t.pb_raw_qty_lb::text || ' lb here; a negative balance '
                || 'cannot be a FIFO layer, so this comes in at zero and wants a physical count. '
              ELSE '' END
...
BEGIN
    ...
    SELECT COALESCE(SUM(qty_lb_remaining),0), COALESCE(SUM(product_cost),0), count(*)
      INTO v_lb, v_usd, v_items FROM public.feed_receipts;
    SELECT count(*) INTO v_zeroed FROM target WHERE pb_raw_qty_lb < 0;

    RAISE NOTICE 'Re-seeded to PB 2026-08-30: % layers, % lb, $%. % PB negative(s) floored to zero.',
        v_items, to_char(v_lb,'FM999,999,999'), to_char(v_usd,'FM999,999,990.00'), v_zeroed;
END
$verify$;
